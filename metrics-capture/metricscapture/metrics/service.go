...
func NewService(cfg Config, uploader Uploader) *Service {
	...
	autoFlushTimeout := cfg.AutoFlushTimeout
	if autoFlushTimeout <= 0 {
		autoFlushTimeout = 3 * time.Second
	}
	return &Service{
		...
		autoFlushTimeout: autoFlushTimeout,
	}
}
...
func (s *Service) EmitPayload(name string, value float64, tags map[string]string, payload map[string]any) {
	if strings.TrimSpace(name) == "" {
		return
	}
	s.emitMetric(Metric{
...
func (s *Service) emitMetric(metric Metric) {
	s.queue.enqueue(metric)
	if s.autoFlushOnEmit {

...
`metrcics capture` is a starter Viam module for **arbitrary, push-style metrics capture**.
...
# Optional if you do not set attributes.part_id:
export VIAM_PART_ID="your-part-id"
```
...
svc, err := metricscapture.NewModuleServiceFromEnv(context.Background(), logger, metricscapture.StartupConfig{
    // Optional if VIAM_PART_ID is set in env:
    PartID:        "YOUR_PART_ID",
...
### 2) Send or flush data with DoCommand

Upload all queued metrics now:
...
Send one payload and upload it immediately:

```json
{
  "command": "record_payload_now",
  "name": "metricscapture.custom_event",
  "value": 1,
  "tags": {
    "source": "manual"
  },
...
| Command | Required fields | Optional fields | What it does | Typical response keys |
| --- | --- | --- | --- | --- |
| `metrics_flush_now` | `command` | none | Uploads all currently queued metrics to Viam Data Client now. | `status`, `uploaded_count`, `file_ids`, `error` |
| `metrics_stats` | `command` | none | Returns module health/state counters. | `status`, `queue_depth`, `dropped_count`, `uploaded_count`, `upload_fail_count`, `last_upload_at`, `last_upload_error`, `max_queue_size`, `batch_size` |
| `record_payload_now` | `command`, `name` | `value` (number), `tags` (object of strings), `payload` (object) | Emits one event metric with payload and immediately flushes it. | `status`, `uploaded_count`, `file_ids`, `error` |

### 2-minute validation sequence

Run these commands in order from any client that can call `DoCommand` on this service:
...
2) Emit and store one payload immediately:

```json
{
  "command": "record_payload_now",
  "name": "metricscapture.quickcheck",
  "value": 1,
  "tags": {
    "source": "readme_test"
  },
...
- `uploaded_count` increases
- `upload_fail_count` does not increase
- `last_upload_error` is empty
...
export VIAM_API_KEY="your-api-key"
export VIAM_API_KEY_ID="your-api-key-id"
# Optional if part_id is omitted from attributes:
export VIAM_PART_ID="your-part-id"
```
...
Or send and store a payload in one command:
...
### Optional: automatic push on each event

If you want every emitted metric to upload immediately (without calling `metrics_flush_now`), set:
...
When enabled, each `EmitMetric(...)` triggers an immediate upload attempt with the configured timeout.
